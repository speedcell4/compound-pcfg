...
def get_data(args):
    ...
    def convert(textfile, lowercase, replace_num,
        ...
        # Write output
        f = {"source": sents, "other_data": other_data, "batch_l": np.array(batch_l, dtype=int),
             "source_l": np.array(batch_w, dtype=int), "sents_l": np.array(sent_l, dtype=int),
             "batch_idx": np.array(batch_idx[:-1], dtype=int), "vocab_size": np.array([len(indexer.d)]),
             "idx2word": indexer.idx2word, "word2idx": {word: idx for idx, word in indexer.idx2word.items()}}
