...
def main(args):
    ...
    print(
        f'Train: {train_data.sents.size(0):d} sents / {len(train_data):d} batches, Val: {val_data.sents.size(0):d} sents / {len(val_data):d} batches')
    print(f'Vocab size: {vocab_size:d}, Max Sent Len: {max_len:d}')
...
def eval(data, model):
    ...
    print(f'ReconPPL: {recon_ppl:.2f}, KL: {kl:.4f}, PPL (Upper Bound): {ppl_elbo:.2f}')
    print(f'Corpus F1: {corpus_f1 * 100:.2f}, Sentence F1: {sent_f1 * 100:.2f}')
    model.train()
